...
function DocumentConversion(options, comments) {
    ...
    function convertToHtml(document) {
        ...
        return promises.mapSeries(deferredNodes, function(deferred) {
            ...
            let cleanedHTML = writer.asString();
            if (cleanedHTML.includes("&lt;math ")) {
                while (cleanedHTML.includes("&lt;math ")) {
                    let idx = cleanedHTML.indexOf("&lt;math ");
                    let endIdx = cleanedHTML.indexOf("&lt;/math&gt;", idx) + "&lt;/math&gt;".length;
                    if (endIdx < 0) {
                        let originalHTML = cleanedHTML.substring(idx);
                        let convertedHTML = originalHTML.replace(/&lt;/g, "<").replace(/&gt;/g, ">");
                        cleanedHTML = cleanedHTML.replace(originalHTML, convertedHTML);
                        break;
                    }
            
                    let originalHTML = cleanedHTML.substring(idx, endIdx);
                    let convertedHTML = originalHTML.replace(/&lt;/g, "<").replace(/&gt;/g, ">");
                    cleanedHTML = cleanedHTML.replace(originalHTML, convertedHTML);
                }
            }
            return new results.Result(cleanedHTML, messages);
        });
    }
